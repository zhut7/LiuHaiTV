...
**想改代码 / 接手维护？请先读 [`AGENTS.md`](AGENTS.md)** —— 那里有架构总览、各模块职责、
「为什么这么设计」的决策记录，以及一份**雷区清单**（哪些地方改了会炸）。
...
├─ AGENTS.md                     交接文档：架构、设计决策、雷区（**改代码前请先读它**）
│
...
全部离线（临时库 + offscreen），**退出码 0 即通过**；共 **288 条断言**。

| 脚本                 | 覆盖                                                      | 断言  |
| ------------------ | ------------------------------------------------------- | --- |
| `verify_data.py`   | 数据模型 / M3U 解析 / 搜刮落库 / 核心白名单 / 身份归键 / 精简计划与幂等 / 备份      | 64  |
| `verify_player.py` | mpv 封装 / failover 状态机 / 健康源优先 / 异步健康检测                  | 17  |
| `verify_ui.py`     | 主窗口分组 / 点击判定四态 / 源管理弹窗 / 三态配色 / 置顶 / 音量记忆 / 刷新源 / 右键不换台 | 95  |
| `verify_sync.py`   | 加速器改写 / 同步对账语义 / 新增频道三档 / 弹窗                            | 52  |
| `verify_naming.py` | 频道名规范化 / 改名脚本的冲突保护                                      | 60  |
